...
const Navbar = () => {
  ...
  <div className=' absolute top-0 right-0 pt-16 text-base font-medium text-gray-600 hidden z-20 group-hover:block'>
   <div className=' min-w-48 bg-stone-100 rounded flex flex-col gap-4 p-4' > 
    ...
    <p onClick={()=>setToken(false)} className=' hover:text-black cursor-pointer'>Login</p>

  </div>
</div>
          </div>
          
          :  <button onClick={()=>navigate('/login')} className=' bg-primary  text-white px-8 py-3  rounded-full font-light  hidden md:block'>Contact Account</button>
